#+TITLE: ox-lexical: Org-mode to Ghost Lexical JSON Exporter
#+AUTHOR: ii.coop
#+DATE: 2026-01-28

* Overview

=ox-lexical= is an Emacs org-mode export backend that converts org files to
...
Load the elisp file in Emacs:

#+BEGIN_SRC elisp
(load "/path/to/ox-lexical/ox-lexical.el")
#+END_SRC
...
| File                 | Purpose                                    |
|----------------------+--------------------------------------------|
| =ox-lexical.el=      | Emacs org-mode export backend              |
...
For a complete publishing workflow with media generation and enrichment, see =ghost-publish.el=.
...
See [[file:ghost-publish.org][ghost-publish.org]] for complete documentation.
